...
if __name__ == '__main__':
    ...
    for name, group in grouped:
        ...
        train_df = train_df.append(group[:30])
    ...
    outpath = os.path.join('plots', 'iris')
        ...
        for hidden in itertools.product([i for i in range(3, 7)], repeat=i):
            ...
            for n in n_list:
                ...
                for it in range(5):
                    ...
                    actual_precision = precision_score(y_test.values, p_y.values, average='micro')
            ...
            conf_m = confusion_matrix(y_test.values, p_y.values)
            print('##### Iris Experiment - Layers {}\n'.format(hidden))
            print('Weights:')
    ...
    conf_m = confusion_matrix(y_validation.values, p_y.values)
    print('##### Iris Experiment - Best Topology {}\n'.format(best_topology))
    print('Weights:')
    print(best_mlp.w)
    print()
    print('Best n:')
    ...
    actual_precision = precision_score(y_validation.values, p_y.values, average='micro')
